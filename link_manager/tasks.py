...
def main_task():
    ...
    Эта задача вызывает `web_data_fetcher` для каждой активной ссылки в базе
    данных.
    """
...
def web_data_fetcher(link_id):
    ...
    Если ссылка существует и активна, эта задача отправляет HTTP-запрос по URL
    ссылки. В зависимости от типа условия (json, text или status_code),
    проверяется ответ на соответствие критериям. Результат проверки сохраняется
    в объекте Link в полях `timestamp` и `is_result`.
    """
    try:
